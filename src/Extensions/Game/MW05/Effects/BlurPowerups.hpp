...
namespace Extensions::Game::MW05::Effects {
  class BlurPowerups : public IGameEffect {
    ...
    class BoltPowerUp : public IPowerUp {
      std::vector<OpenMW::PVehicle*> mSpawnedBolts;
      ...
      bool sendBolt(Direction boltDirection) {
        ...
        // Create new PVehicle
        auto* new_pvehicle = OpenMW::PVehicle::Construct(OpenMW::VehicleParams(OpenMW::DriverClass::None, OpenMW::Attrib::StringToKey("trafpizza"), direction,
                                                                               position, nullptr, OpenMW::eVehicleParamFlags::SnapToGround)) |
        ...
        mSpawnedBolts.push_back(new_pvehicle);
     ...
     public:
      ...
      virtual void DoCleanUp() override {
        ...
        for (auto it = std::cbegin(mSpawnedBolts); it != std::cend(mSpawnedBolts);) {
    ...
    class BargePowerUp : public IPowerUp {
      ...
      bool pushObjectsAway(float radius = 80.0f, float force = 60.0f) {
        ...
        auto* player_pvehicle = OpenMW::PVehicleEx::GetPlayerInstance();
        if (!player_pvehicle) return false;

        const auto& player_pos      = player_pvehicle->GetPosition();
        const auto  danger_zone_min = player_pos - OpenMW::UMath::Vector3(radius, radius, radius);
        const auto  danger_zone_max = player_pos + OpenMW::UMath::Vector3(radius, radius, radius);
    ...
    ImGuiKey mActivateKey;
    ImGuiKey mSwitchKey;
    ImGuiKey mActivateBackwardsKey;
    ...
    bool hasRequestedSwitchPowerup() { return ImGui::IsKeyReleased(mSwitchKey); }
    bool hasRequestedActivatePowerup() { return ImGui::IsKeyReleased(mActivateKey); }
    bool hasRequestedActivatePowerupBackward() { return ImGui::IsKeyReleased(mActivateBackwardsKey); }
   ...
   protected:
    ...
    virtual void _drawConfig(const ImGuiViewport*) noexcept override {
      static constexpr char CONST_BLURPOWERUPS_ACTIVATE[]           = "Activate: ";
      static constexpr char CONST_BLURPOWERUPS_SWITCH[]             = "Switch: ";
      static constexpr char CONST_BLURPOWERUPS_ACTIVATE_BACKWARDS[] = "Activate Backwards: ";

      static float offset_x = 0.0f;

      ImGui::TextUnformatted(CONST_BLURPOWERUPS_ACTIVATE);
      ImGui::SameLine();
      {
        ImGui::SetCursorPosX(offset_x);
        ImGui::SetNextItemWidth(offset_x * 2.0f);
        if (ImGui::KeyCatcher("##BLURPOWERUPSCONFIG1", &mActivateKey)) {
          Config::Get()["Effects"][mIndex]["Config"]["ActivateKeyID"].SetInt(mActivateKey);
          Config::Get().Save();
        }
      }

      ImGui::TextUnformatted(CONST_BLURPOWERUPS_SWITCH);
      ImGui::SameLine();
      {
        ImGui::SetCursorPosX(offset_x);
        ImGui::SetNextItemWidth(offset_x * 2.0f);
        if (ImGui::KeyCatcher("##BLURPOWERUPSCONFIG2", &mSwitchKey)) {
          Config::Get()["Effects"][mIndex]["Config"]["SwitchKeyID"].SetInt(mSwitchKey);
          Config::Get().Save();
        }
      }

      ImGui::TextUnformatted(CONST_BLURPOWERUPS_ACTIVATE_BACKWARDS);
      ImGui::SameLine();
      offset_x = ImGui::GetCursorPosX();
      {
        ImGui::SetCursorPosX(offset_x);
        ImGui::SetNextItemWidth(offset_x * 2.0f);
        if (ImGui::KeyCatcher("##BLURPOWERUPSCONFIG3", &mActivateBackwardsKey)) {
          Config::Get()["Effects"][mIndex]["Config"]["ActivateBackwardsKeyID"].SetInt(mActivateBackwardsKey);
          Config::Get().Save();
        }
      }
    }
   ...
   public:
    explicit BlurPowerups() : IGameEffect(64), mActivePowerUp(PowerUpType::Boost) {
      ...
      mActivateKey          = Config::Get()["Effects"][mIndex]["Config"]["ActivateKeyID"].GetInt();
      mSwitchKey            = Config::Get()["Effects"][mIndex]["Config"]["SwitchKeyID"].GetInt();
      mActivateBackwardsKey = Config::Get()["Effects"][mIndex]["Config"]["ActivateBackwardsKeyID"].GetInt();
    }
  };
}  // namespace Extensions::Game::MW05::Effects
