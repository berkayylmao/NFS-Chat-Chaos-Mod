...
namespace Extensions::Game::Carbon::Effects {
  class DoABarrelRoll : public IGameEffect {
   protected:
    virtual bool _activate() noexcept override {
      OpenCarbon::PVehicleEx::ForEachInstance([=](OpenCarbon::PVehicle* pvehicle) {
        if (auto* rb = pvehicle->GetRigidBody()) {
          const auto&                angular_velocity = rb->GetAngularVelocity();
          OpenCarbon::UMath::Vector3 direction;
          rb->GetForwardVector(direction);
          rb->SetAngularVelocity(angular_velocity + OpenCarbon::UMath::Vector3(15.0f * direction.x, 15.0f * direction.y, 0.0f));
        }
      });
      return true;
    }
   ...
   public:
    explicit DoABarrelRoll() : IGameEffect(7) {}
  };
}  // namespace Extensions::Game::Carbon::Effects
